...
#####################################
#Fill these:
IL2_exclusive <- read.table("IL2_exclusive.txt", header = TRUE)
IL2_independent <- read.table("IL2_independent.txt", header = TRUE)
IL15_exclusive <- read.table("IL15_exclusive.txt", header = TRUE)
IL15_independent <- read.table("IL15_independent.txt", header = TRUE)
for (i in 1:4){IL2_exclusive[,i] <- as.character(IL2_exclusive[,i])}
for (i in 1:4){IL2_independent[,i] <- as.character(IL2_independent[,i])}
for (i in 1:4){IL15_exclusive[,i] <- as.character(IL15_exclusive[,i])}
...
j=2
for (i in 5:8){
tmp_1 <- nchar(as.character(IL2_exclusive$cdr3aa))
tmp_2 <- IL2_exclusive[,i]
...
j=2
for (i in 5:8){
  tmp_1 <- nchar(as.character(IL15_exclusive$cdr3aa))
  tmp_2 <- IL15_exclusive[,i]
  tmp_3 <- as.data.frame(cbind(tmp_1, tmp_2))
  colnames(tmp_3) <- c("Len", "counts")
  tmp_4 <- aggregate(counts ~ Len, data=tmp_3, sum)
  tmp_4$counts <- (tmp_4$counts/sum(tmp_4$counts))*100
...
tmp_1$strain <- rep("IL2_exclusive", nrow(tmp_1))
...
tmp_1 <- as.data.frame(tmp_1)
tmp_1$strain <- rep("IL15_exclusive", nrow(tmp_1))
colnames(tmp_1) <- c("Len", "mean", "SD", "pValue", "strain")
...
Data_IL2_means[,4] <- c("IL2_exclusive", "IL2_independent")
#
tmp_1 <- Data_IL2[,1:5]
...
Data_IL15_means[,4] <- c("IL15_exclusive", "IL15_independent")
#
tmp_1 <- Data_IL15[,1:5]
tmp_means_1 <- matrix(data=NA, ncol=1, nrow=4)
for (i in 1:4) {tmp_means_1[i,1] <- sum(tmp_1[,1]*tmp_1[,i+1]) / (sum(tmp_1[,1]))} 
...
ggplot(Data_IL2_graph, aes(x = Len, y = mean, fill = strain))+ 
  ...
  #annotate("segment", x = as.numeric(stars_signif_IL2[,1])-0.7, xend = as.numeric(stars_signif_IL2[,1])+0.7, y = as.numeric(stars_signif_IL2[,5]), yend = as.numeric(stars_signif_IL2[,5]), colour = "black", size=0.2)+
  ggsave("Spectratype_IL2_Excl_vs_indep.pdf", width=10, height=10, units = "cm")
...
ggplot(Data_IL15_graph, aes(x = Len, y = mean, fill = strain))+ 
  ...
  #annotate("segment", x = as.numeric(stars_signif_IL15[,1])-0.7, xend = as.numeric(stars_signif_IL15[,1])+0.7, y = as.numeric(stars_signif_IL15[,5]), yend = as.numeric(stars_signif_IL15[,5]), colour = "black", size=0.2)+
  ggsave("Spectratype_IL15_Excl_vs_indep.pdf", width=10, height=10, units = "cm")
...
ggplot(Data_IL2_means, aes(x = coordinate, y = mean, fill = condition))+ 
  ...
  scale_x_continuous(breaks=c(1.2,2.2), label = c("", ""))+
  ...
  ggsave("Spectratype_mean_IL2_simple_Excl_vs_indep.tiff", width=8, height=11, units = "cm")
...
ggplot(Data_IL15_means, aes(x = coordinate, y = mean, fill = condition))+ 
  ...
  ggsave("Spectratype_mean_IL15_simple_Excl_vs_indep.tiff", width=8, height=11, units = "cm")
