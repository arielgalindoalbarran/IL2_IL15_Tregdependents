...
#READ file:
IL2_dependent <- read.table("IL2_dependent.txt", sep ="\t", header = TRUE, comment="", quote="")
IL15_dependent <- read.table("IL15_dependent.txt", sep ="\t", header = TRUE, comment="", quote="")
...
j=1
for (i in 5:8) {
sum_counts_WT[j,2] <- c(sum(IL2_dependent[,i])+sum(IL2_independent[,i]))
j=j+1
}
...
###############
#IL2 dep
condition= IL2_dependent
tmp <- condition
tmp_1 <- as.data.frame(tmp[,c(2,5)])
tmp_1 <- aggregate(WT_1 ~ v, data=tmp_1, sum)
tmp_1[,2] <- (tmp_1[,2]/sum(tmp_1[,2]))*100
tmp_2 <- as.data.frame(tmp[,c(2,6)])
tmp_2 <- aggregate(WT_2 ~ v, data=tmp_2, sum)
tmp_2[,2] <- (tmp_2[,2]/sum(tmp_2[,2]))*100
tmp_3 <- as.data.frame(tmp[,c(2,7)])
tmp_3 <- aggregate(WT_4 ~ v, data=tmp_3, sum)
tmp_3[,2] <- (tmp_3[,2]/sum(tmp_3[,2]))*100
tmp_4 <- as.data.frame(tmp[,c(2,8)])
tmp_4 <- aggregate(WT_8 ~ v, data=tmp_4, sum)
tmp_4[,2] <- (tmp_4[,2]/sum(tmp_4[,2]))*100
Data_IL2_dependent <- cbind(tmp_1, tmp_2[,2], tmp_3[,2], tmp_4[,2])
colnames(Data_IL2_dependent) <- c("V","WT1", "WT2", "WT3", "WT4")
...
tmp_4 <- aggregate(WT_8 ~ v, data=tmp_4, sum)
tmp_4[,2] <- (tmp_4[,2]/sum(tmp_4[,2]))*100
Data_IL15_dependent <- cbind(tmp_1, tmp_2[,2], tmp_3[,2], tmp_4[,2])
colnames(Data_IL15_dependent) <- c("V","WT1", "WT2", "WT3", "WT4")
...
# Mean and SD #
###############
Data_IL2_dependent$mean <- rowMeans(Data_IL2_dependent[,2:5])
Data_IL2_independent$mean <- rowMeans(Data_IL2_independent[,2:5])
Data_IL15_dependent$mean <- rowMeans(Data_IL15_dependent[,2:5])
Data_IL15_independent$mean <- rowMeans(Data_IL15_independent[,2:5])
#
Data_IL2_dependent$SD <- rowSds(as.matrix(Data_IL2_dependent[,2:5]))
Data_IL2_independent$SD <- rowSds(as.matrix(Data_IL2_independent[,2:5]))
Data_IL15_dependent$SD <- rowSds(as.matrix(Data_IL15_dependent[,2:5]))
...
# IL2 #
#######
tmp_1 <- setdiff(Data_IL2_dependent[,1], Data_IL2_independent[,1]) 
tmp_2 <- matrix(data = 0, ncol = ncol(Data_IL2_dependent) , nrow= length(tmp_1))
tmp_2[,1] <- tmp_1
colnames(tmp_2) <- colnames(Data_IL2_dependent)
...
#
tmp_1 <- setdiff(Data_IL2_independent[,1], Data_IL2_dependent[,1]) 
tmp_2 <- matrix(data = 0, ncol = ncol(Data_IL2_independent) , nrow= length(tmp_1))
tmp_2[,1] <- tmp_1
colnames(tmp_2) <- colnames(Data_IL2_independent)
Data_IL2_dependent <- rbind(Data_IL2_dependent, tmp_2)
Data_IL2_dependent[,1] <- as.character(Data_IL2_dependent[,1])
Data_IL2_dependent <- Data_IL2_dependent[order(Data_IL2_dependent[,1]), ]
for(i in 2:7){Data_IL2_dependent[,i] <- as.numeric(Data_IL2_dependent[,i])}
...
rownames(Data_IL2_dependent) <- NULL
rownames(Data_IL2_independent) <- NULL
index <- match(Data_IL2_independent[,1], Data_IL2_dependent[,1], nomatch = 0)
Data_IL2_dependent <- Data_IL2_dependent[index,]
rownames(Data_IL2_dependent) <- NULL
...
# IL15 #
########
tmp_1 <- setdiff(Data_IL15_dependent[,1], Data_IL15_independent[,1]) 
tmp_2 <- matrix(data = 0, ncol = ncol(Data_IL15_dependent) , nrow= length(tmp_1))
tmp_2[,1] <- tmp_1
colnames(tmp_2) <- colnames(Data_IL15_dependent)
...
#
tmp_1 <- setdiff(Data_IL15_independent[,1], Data_IL15_dependent[,1]) 
tmp_2 <- matrix(data = 0, ncol = ncol(Data_IL15_independent) , nrow= length(tmp_1))
tmp_2[,1] <- tmp_1
colnames(tmp_2) <- colnames(Data_IL15_independent)
Data_IL15_dependent <- rbind(Data_IL15_dependent, tmp_2)
Data_IL15_dependent[,1] <- as.character(Data_IL15_dependent[,1])
Data_IL15_dependent <- Data_IL15_dependent[order(Data_IL15_dependent[,1]), ]
for(i in 2:7){Data_IL15_dependent[,i] <- as.numeric(Data_IL15_dependent[,i])}
...
rownames(Data_IL15_dependent) <- NULL
rownames(Data_IL15_independent) <- NULL
index <- match(Data_IL15_independent[,1], Data_IL15_dependent[,1], nomatch = 0)
Data_IL15_dependent <- Data_IL15_dependent[index,]
rownames(Data_IL15_dependent) <- NULL
...
######
#IL2_dependent
IL2_dependent_graphic <- matrix(data = NA, ncol = 6, nrow = nrow(Data_IL2_dependent))
colnames(IL2_dependent_graphic) <- c("x_values", "mean", "SD", "V_segment", "condition", "pValue")
IL2_dependent_graphic[,2] <- c(Data_IL2_dependent[,6])
IL2_dependent_graphic[,3] <- Data_IL2_dependent[,7]
IL2_dependent_graphic[,4] <- Data_IL2_dependent[,1]
IL2_dependent_graphic[,5] <- rep("IL-2 Dependent",nrow(Data_IL2_dependent))
#Add X
IL2_dependent_graphic[,1] <- seq(from=1, to=nrow(Data_IL2_dependent)*3, by=3)
#pValues
for (i in 1:nrow(Data_IL2_dependent)) {
  if (as.numeric(Data_IL2_dependent[i,6]) > as.numeric(Data_IL2_independent[i,6]))
  {pValue <- wilcox.test(as.numeric(Data_IL2_dependent[i,2:5]),as.numeric(Data_IL2_independent[i,2:5]),  alternative = "g", paired = FALSE)
  }else {pValue <- wilcox.test(as.numeric(Data_IL2_dependent[i,2:5]),as.numeric(Data_IL2_independent[i,2:5]),  alternative = "l", paired = FALSE)}
  IL2_dependent_graphic[i,6] <- round(pValue$p.value, digits = 4)}
...
# Together
IL2_graphic <- as.data.frame(rbind(IL2_independent_graphic, IL2_dependent_graphic))
...
######
#IL15_dependent
IL15_dependent_graphic <- matrix(data = NA, ncol = 6, nrow = nrow(Data_IL15_dependent))
colnames(IL15_dependent_graphic) <- c("x_values", "mean", "SD", "V_segment", "condition", "pValue")
IL15_dependent_graphic[,2] <- c(Data_IL15_dependent[,6])
IL15_dependent_graphic[,3] <- Data_IL15_dependent[,7]
IL15_dependent_graphic[,4] <- Data_IL15_dependent[,1]
IL15_dependent_graphic[,5] <- rep("IL-15 Dependent",nrow(Data_IL15_dependent))
#Add X
IL15_dependent_graphic[,1] <- seq(from=1, to=nrow(Data_IL15_dependent)*3, by=3)
#pValues
for (i in 1:nrow(Data_IL15_dependent)) {
  if (as.numeric(Data_IL15_dependent[i,6]) > as.numeric(Data_IL15_independent[i,6]))
  {pValue <- wilcox.test(as.numeric(Data_IL15_dependent[i,2:5]),as.numeric(Data_IL15_independent[i,2:5]),  alternative = "g", paired = FALSE)
  }else {pValue <- wilcox.test(as.numeric(Data_IL15_dependent[i,2:5]),as.numeric(Data_IL15_independent[i,2:5]),  alternative = "l", paired = FALSE)}
  IL15_dependent_graphic[i,6] <- round(pValue$p.value, digits = 4)}
...
IL15_independent_graphic[,5] <- rep("IL-15 Independent",nrow(Data_IL15_independent))
#Add X
IL15_independent_graphic[,1] <- seq(from=2, to=nrow(Data_IL15_independent)*3, by=3)
# Together
IL15_graphic <- as.data.frame(rbind(IL15_independent_graphic, IL15_dependent_graphic))
...
ggplot(IL2_graphic_selected, aes(x = x_values, y = mean, fill=condition))+ 
  ...
  ggsave("Vseg_IL2_dep_indep.pdf", width=10, height=10, units = "cm")
...
ggplot(IL15_graphic_selected, aes(x = x_values, y = mean, fill=condition))+ 
  ...
  ggsave("Vseg_IL15_dep_indep.pdf", width=10, height=10, units = "cm")
