...
#
echo 'Filtering to sequences with at least one representative reads'
SplitSeq.py group -s $PREFIX_NAME-C_collapse-unique.fastq -f CONSCOUNT --num 1 --outname $PREFIX_NAME-C_1;
echo 'Creating an annotation table'
ParseHeaders.py table -s $PREFIX_NAME-C_1_atleast-1.fastq -f ID CREGION CONSCOUNT DUPCOUNT;
echo 'Align with TCR B using MiXCR'
mixcr align -t 10 --library imgt --species mmu -c $TRChain -r $PREFIX_NAME-align_1.log.txt $PREFIX_NAME-C_1_atleast-1.fastq $PREFIX_NAME-alignments_1.vdjca;
mixcr assemble -t 10 -r $PREFIX_NAME-assemble_1.log.txt $PREFIX_NAME-alignments_1.vdjca $PREFIX_NAME-clones_1.clns;
mixcr exportClones $PREFIX_NAME-clones_1.clns $PREFIX_NAME-clones_1.txt;
echo 'Export to VDJtools'
java -jar /home/agalindo/work/vdjtools/vdjtools-1.1.10.jar Convert -S mixcr $PREFIX_NAME-clones_1.txt vdjtools
#STATISTICS:
...
printf "Number of MIGs Post- filter at least 1 read:\n" >> Statistics_$PREFIX_NAME.txt;
awk '{s++}END{print s/4}' $PREFIX_NAME-C_1_atleast-1.fastq >> Statistics_$PREFIX_NAME.txt;
...
printf "MIGS Aligned to TCR locus, and used to clonotypes, at least 1 read:\n" >> Statistics_$PREFIX_NAME.txt;
awk '{print $1 }' vdjtools.$PREFIX_NAME-clones_1.txt | awk '{s+=$1} END {printf "%.0f\n", s}' >> Statistics_$PREFIX_NAME.txt;
...
printf "Number of clonotypes, at least 1 read:\n" >> Statistics_$PREFIX_NAME.txt;
awk '{print $1 }' vdjtools.$PREFIX_NAME-clones_1.txt | wc -l >> Statistics_$PREFIX_NAME.txt;
...
printf "Aligment percentages, at least 1 read:\n" >> Statistics_$PREFIX_NAME.txt;
sed -n 8p $PREFIX_NAME-align_1.log.txt >> Statistics_$PREFIX_NAME.txt;
sed -n 9p $PREFIX_NAME-align_1.log.txt >> Statistics_$PREFIX_NAME.txt;
sed -n 10p $PREFIX_NAME-align_1.log.txt >> Statistics_$PREFIX_NAME.txt;
sed -n 11p $PREFIX_NAME-align_1.log.txt >> Statistics_$PREFIX_NAME.txt;
sed -n 16p $PREFIX_NAME-align_1.log.txt >> Statistics_$PREFIX_NAME.txt;
...
grep 'CONSCOUNT' $PREFIX_NAME-C_1_atleast-1.fastq | awk -F "|" '{print $3}' | awk -F "=" '{print $2}' |  sort -n -r > $PREFIX_NAME-umiVSreads_atleast-1.csv;
gzip *.fastq;
gzip *.log;
gzip *.tab;
gzip *.vdjca; 
...
Rscript $WorkDir/Rscripts/Histograms.R $PREFIX_NAME $PREFIX_Cond2 $PREFIX_NAME-umiVSreads_atleast-1.csv $PREFIX_NAME-umiVSreads_atleast-2.csv $PREFIX_Cond2-umiVSreads_atleast-1.csv $PREFIX_Cond2-umiVSreads_atleast-2.csv vdjtools.$PREFIX_NAME-clones_1.txt vdjtools.$PREFIX_NAME-clones_2.txt vdjtools.$PREFIX_Cond2-clones_1.txt vdjtools.$PREFIX_Cond2-clones_2.txt;


#Move files:
mkdir MoreThan1reads;
mkdir MoreThan2reads;
mv DivStats_2* MoreThan2reads;
mv DivStats_1* MoreThan1reads;
mv *UMIvsReads_2read.svg MoreThan2reads;
mv *UMIvsClonotype_2read.svg MoreThan2reads;
mv *UMIvsReads_1read.svg MoreThan1reads;
mv *UMIvsClonotype_1read.svg MoreThan1reads;
rm *segments.txt;
unset VDJt;
unset PREFIX_NAME;
unset PREFIX_Cond2;
